<!-- ===== Footer ===== -->
  <footer class="bg-gradient-to-b from-blue-400 to-indigo-500 text-[#F7EDD6] rounded-t-3xl py-4 px-6 md:px-10 mt-auto
  md:ml-[320px] md:mr-3 transition-all duration-300">
    <div class="flex flex-col md:flex-row items-center md:items-start justify-between gap-3 md:gap-0">

<div class="flex items-center space-x-3">
<img src="{{asset('assets/logo.png')}}"
     alt="Logo Footer Dashboard"
     class="w-12 h-12 object-contain">

        <div class="leading-tight">
          <p class="text-[12px] sm:text-sm font-semibold">
            SarprasKu
          </p>
          <p class="text-[11px] sm:text-xs">
            SISTEM PEMINJAMAN ALAT SARPRAS
          </p>
        </div>
      </div>

      <!-- Kanan -->
      <div class="text-right text-[11px] sm:text-xs md:text-sm leading-snug">
        <p>© 2026 Sarprasku – Sistem Peminjaman Alat Sarpras</p>
        <p>Jl. Pagelaran, Bogor | Telp: (0895)355310172</p>
      </div>

    </div>
  </div>
  </footer>
